...
VIEW_MODEL_WORKERS = int(os.environ.get("VIEW_MODEL_WORKERS", 10))
...
def prepare_view_models(items: Iterable[NewsItem], translate_enabled: bool) -> List[dict]:
    worker = partial(_process_view_model_item, translate_enabled)
    return list(VIEW_MODEL_EXECUTOR.map(worker, items))
