...
def _prepare_single_view_model(item: NewsItem, translate_enabled: bool) -> dict:
...
def prepare_view_models(items: Iterable[NewsItem], translate_enabled: bool) -> List[dict]:
    with ThreadPoolExecutor() as executor:
        return list(
            executor.map(lambda item: _prepare_single_view_model(item, translate_enabled), items)
        )
