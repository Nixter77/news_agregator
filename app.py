...
CACHE_DIR   = pathlib.Path("/tmp/rss_cache"); CACHE_DIR.mkdir(exist_ok=True)
# Define paths relative to the app's location
APP_DIR = pathlib.Path(__file__).parent if "__file__" in locals() else pathlib.Path.cwd()
...
if not news_items:
    ...
    for n in news_items:
        ...
        st.image(pictogram_img, use_container_width=True)
