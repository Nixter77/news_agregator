...
def prepare_view_models(items: Iterable[NewsItem], translate_enabled: bool) -> List[dict]:
    # Use ThreadPoolExecutor to parallelize translation and image generation
    # Max workers set to 20 to handle IO-bound tasks efficiently
    with ThreadPoolExecutor(max_workers=20) as executor:
        view_models = list(executor.map(lambda item: _process_view_model(item, translate_enabled), items))
    return view_models
